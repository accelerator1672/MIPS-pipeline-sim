...
void ID_Stage()
{
    ...
    */
    ...
    if (!OPCODE(instr) && FUNC(instr) != 0x8)
    { /*CASE FOR R-TYPE INST*/
    ...
    else if (OPCODE(instr) > 3)
    { /*I-TYPE HERE REG2 DOESNT MATTER*/
        ...
        case (0xb):
            ...
            break;
        case (0x2b):
            CURRENT_STATE.ID_EX_ALU_OP = 1; /*CASE FOR SW HAVE TO EXPLICITELY SET REG_WRITE = 0*/
            CURRENT_STATE.ID_EX_REG_WRITE = 0;
...
void EX_Stage()
{
    if (CURRENT_STATE.EX_MEM_FORWARD_REG1 && CURRENT_STATE.ID_EX_REG_RS &&
        CURRENT_STATE.EX_MEM_FORWARD_REG1 == CURRENT_STATE.ID_EX_REG_RS)
        CURRENT_STATE.ID_EX_REG1 = CURRENT_STATE.EX_MEM_FORWARD_VALUE1;
    ...
    if (CURRENT_STATE.EX_MEM_FORWARD_REG2 && CURRENT_STATE.ID_EX_ALU_SRC &&
        CURRENT_STATE.EX_MEM_FORWARD_REG2 == CURRENT_STATE.ID_EX_REG_RT)
        CURRENT_STATE.ID_EX_REG2 = CURRENT_STATE.EX_MEM_FORWARD_VALUE2;
    ...
    else
    {
        ...
        {
        case (0):
            CURRENT_STATE.EX_MEM_ALU_OUT = CURRENT_STATE.ID_EX_REG1 - OPERAND2;
            break;
        case (1):
            CURRENT_STATE.EX_MEM_ALU_OUT = CURRENT_STATE.ID_EX_REG1 + OPERAND2;
            break;
        ...
        case (4):
            ...
            break;
        case (5):
            OPERAND2 = CURRENT_STATE.ID_EX_SHAMT;
        ...
        case (7):
            ...
            break;
        case (8):
            CURRENT_STATE.EX_MEM_ALU_OUT = OPERAND2 << 16;
            break;
        }
    }
...
void WB_Stage()
{
    ...
    if (CURRENT_STATE.MEM_WB_NPC == (MEM_TEXT_START + 4 * (NUM_INST - 1)))
        RUN_BIT = 0;
}
/***************************************************************/
/*                                                             */
...
void process_instruction()
{
    ...
    if (RUN_BIT)
    {
        ...
        {
            if (!CURRENT_STATE.PIPE_START[3])
                CURRENT_STATE.PIPE_START[3] = 1;
        ...
        {
            if (!CURRENT_STATE.PIPE_START[2])
                CURRENT_STATE.PIPE_START[2] = 1;
        ...
        if (CURRENT_STATE.PC < MEM_TEXT_START + 4 * NUM_INST)
        {
            if (!CURRENT_STATE.PIPE_START[1])
                CURRENT_STATE.PIPE_START[1] = 1;
            CURRENT_STATE.PIPE[0] = CURRENT_STATE.PC;
            if (CURRENT_STATE.PIPE_STALL[0] > 0)
                CURRENT_STATE.PIPE_STALL[0]--;
            else
            {
                IF_Stage();
                CURRENT_STATE.IF_ID_NPC = CURRENT_STATE.PIPE[0];
            }
        }
        else
        {
        ...
        if (!CURRENT_STATE.PIPE_STALL[1] && CURRENT_STATE.ID_EX_MEM_READ && (CURRENT_STATE.ID_EX_DEST == RT(get_inst_info(CURRENT_STATE.IF_ID_NPC)) || CURRENT_STATE.ID_EX_DEST == RS(get_inst_info(CURRENT_STATE.IF_ID_NPC))))
        {
